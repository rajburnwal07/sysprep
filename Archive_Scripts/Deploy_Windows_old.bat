...
set order_location="<location of order.zip>"
::##############################################################
::################### Initializing variables ###################
set "Version=CB_10.1.3"
set current_dir=%cd%
...
::Note: provide css path\name after %SYNERGY_HOME%\css
...
REM if "%~1"=="" (
	...
	REM echo "Command to execute:> Deploy_Windows.bat "D:\CB_10.0\ISO\Portal\CB_10.0\webapps" >> LogCloudBlue.txt"
	REM goto :EOF
REM ) else if "%~1"=="-v" (
	REM echo Script Version: %Version%
	REM goto :EOF
REM )
::Version Checking
if "%~1"=="-v" (
...
:war2war_upgrade
	...
	For %%i in (%css_exception_list%) do (
		echo %%i
		copy /Y "%SYNERGY_HOME%\css\%%i" "%Compare_War_location%\old_war\css\%%i"
		if NOT %ERRORLEVEL% == 0 (
			echo "[Error] Taking backup of Customized %%i failed!"
			goto :pre_deploy_error
		)
	)
	
	echo "Taking backup Customized Images from SYNERGY HOME"
	if exist "%SYNERGY_HOME%\images" copy /Y "%SYNERGY_HOME%\images" "%Compare_War_location%\old_war\"
	if NOT %ERRORLEVEL% == 0 (
	...
	copy /Y "%SYNERGY_HOME%\WEB-INF\grails-app\i18n\messages.properties" "%Compare_War_location%\old_war\WEB-INF\grails-app\i18n\messages.properties"
	if NOT %ERRORLEVEL% == 0 (
	...
	FOR /D %%i IN ("%SYNERGY_HOME%\*") DO (
		if not "%%i"=="%SYNERGY_HOME%\branding" if not "%%i"=="%SYNERGY_HOME%\plugins" if not "%%i"=="%SYNERGY_HOME%\css" if not "%%i"=="%SYNERGY_HOME%\images" if not "%%i"=="%SYNERGY_HOME%\resources" if not "%%i"=="%SYNERGY_HOME%\WEB-INF" ( 
			...
			rd /s /q "%%i"
			if NOT %ERRORLEVEL% == 0 (
				echo "[Error] Deleting %%i failed! from SYNERGY HOME Location"
				goto :error
			)
			
		)
	)
	echo "Deleting OLD files from SYNERGY HOME\WEB-INF"
	...
	echo "Deleting OLD files from SYNERGY HOME\WEB-INF\grails-app"
	...
	For %%i in (%css_exception_list%) do (
		echo %%i
		copy /Y "%Compare_War_location%\old_war\css\%%i" "%SYNERGY_HOME%\css\%%i"
		if NOT %ERRORLEVEL% == 0 (
	...
	rd "%SYNERGY_HOME%\images" /s /q
	if exist "%Compare_War_location%\old_war\images" copy /Y "%Compare_War_location%\old_war\images" "%SYNERGY_HOME%\"
	if NOT %ERRORLEVEL% == 0 (
	...
	copy /Y "%Compare_War_location%\old_war\WEB-INF\grails-app\i18n\messages.properties" "%SYNERGY_HOME%\WEB-INF\grails-app\i18n\messages.properties"
	if NOT %ERRORLEVEL% == 0 (
...
exit /b
::*******************************************************************#
::****************** Restore War Function *********************#
:war2war_upgrade_restore
	echo "Restoring to previous state"
	
	echo Restoring Exsisting WAR from SYNERGY HOME Location
	if exist %SYNERGY_HOME%.war del "%SYNERGY_HOME%.war"
	if exist %SYNERGY_HOME%.war move "%SYNERGY_HOME%_old.war" "%SYNERGY_HOME%.war"
	if NOT %ERRORLEVEL% == 0 (
		echo "[Error] Restoring Customized Images to SYNERGY HOME failed! Restore it manually from location: %Compare_War_location%\old_war\images"
	)
	
	echo "Restoring Customized Images to SYNERGY HOME"
	rd "%SYNERGY_HOME%\images" /s /q
	if exist "%Compare_War_location%\old_war\images" copy /Y "%Compare_War_location%\old_war\images" "%SYNERGY_HOME%\"
	if NOT %ERRORLEVEL% == 0 (
		echo "[Error] Restoring Customized Images to SYNERGY HOME failed! Restore it manually from location: %Compare_War_location%\old_war\images"
	...
	echo "Restoring Customized web.xml to SYNERGY HOME"
	copy /Y "%Compare_War_location%\old_war\WEB-INF\web.xml" "%SYNERGY_HOME%\WEB-INF\web.xml"
	if NOT %ERRORLEVEL% == 0 (
		echo "[Error] Restoring Customized web.xml to SYNERGY HOME failed! Restore it manually from location: %Compare_War_location%\old_war\WEB-INF\web.xml"
	)
	
	echo "Restoring Customized messages.properties to SYNERGY HOME"
	copy /Y "%Compare_War_location%\old_war\WEB-INF\grails-app\i18n\messages.properties" "%SYNERGY_HOME%\WEB-INF\grails-app\i18n\messages.properties"
	if NOT %ERRORLEVEL% == 0 (
		echo "[Error] Restoring messages.properties to SYNERGY HOME failed! Restore it manually from location: %Compare_War_location%\old_war\WEB-INF\grails-app\i18n\messages.properties"
	)
	
	echo Restore Completed. Please check error logs and re-deploy again!
	exit 0
exit /b
::*******************************************************************#
...
:pre_deploy_error
echo Deployment Failed, but no files are hampered.
goto :EOF

:error
echo Deployment Failed, check the error logs and re-deploy again.
