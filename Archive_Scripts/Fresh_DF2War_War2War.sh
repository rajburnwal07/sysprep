...
order_location=D:\\home\\site\\MYDEPLOYMENT\\order.zip

################### User variables ###################
SYNERGY_HOME="D:\\home\\site\\wwwroot\\webapps\\cb"
CATALINA_HOME="D:\\home\\site\\wwwroot"
...
backup(){
	...
	mkdir -p "./BACKUP/$BACKUP_DIRECTORY_NAME"
	cp -r "$SYNERGY_HOME" "./BACKUP/$BACKUP_DIRECTORY_NAME/"

	mv "$SYNERGY_HOME.war" "./BACKUP/$BACKUP_DIRECTORY_NAME/"

	cp -r "$CATALINA_HOME/ESCM-DataFiles/" "./BACKUP/$BACKUP_DIRECTORY_NAME/"
	if [ "$?" != "0" ]; then
