...
order_location="<location of order.zip>"
##############################################################
...
##Note: provide css path/name after $SYNERGY_HOME/css
css_exception_list="merchant.css;merchant_rl.css;modern_ui/organization.css;modern_ui/organization_rl.css"
##############################################################
echo "***********************************************************************************************"
# if [ "$1" == "" ]; then
...
war2war_upgrade(){
	...
	mkdir -p "$Compare_War_location/old_war/css/modern_ui"
	if [ "$?" != "0" ]; then
		echo "[Error] Directory creation failed!"
		exit
	fi
	...
	export IFS=";"
	for word in $css_exception_list; do
 echo $word
		mv "$SYNERGY_HOME/css/$word" "$Compare_War_location/old_war/css/$word"
		if [ "$?" != "0" ]; then
			echo "[Error] Taking backup of Customized $word failed!"
			exit
		fi
	done
	...
	for word in $css_exception_list; do
   echo $word
		mv "$Compare_War_location/old_war/css/$word" "$SYNERGY_HOME/css/$word"
		if [ "$?" != "0" ]; then
			echo "[Error] Replacing Customized $word failed!"
			exit
		fi
	done
