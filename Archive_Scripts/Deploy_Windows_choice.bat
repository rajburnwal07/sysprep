...
set PreValidation_location=%current_dir%\BACKUP\PreValidation_backup
...
echo 2. Final Execution
echo.
set /p choice=Enter your choice:
echo.
if '%choice%'=='1' (
	echo Pre Validation
	goto :Deployment_Status_Check
) else if '%choice%'=='2' (
...
::###### Deploying according to flag value #######
	...
	:CASE_2 
		...
		echo =======================================
		if '%choice%'=='1' (
			echo Pre Validation Upgrade Process Starts
			echo.
			call :backup
				echo 1 > "%CATALINA_HOME%\deployment_status.txt"
			call :extractorder
			call :new_war_copy "%PreValidation_location%", "%PreValidation_location%\%appname%"
			call :df_copy "%PreValidation_location%", "%PreValidation_location%\%appname%"
			call :compare_folder "%Compare_War_location%\DF_WAR_Struct", "%Compare_War_location%\new_war"
			call :rm_df "%PreValidation_location%", "%PreValidation_location%\%appname%"
				echo 2 > "%CATALINA_HOME%\deployment_status.txt"
			GOTO :END
			
		) else (
			echo Final Execution Upgrade Process Starts
			echo.
			if "%ds_value%" == "1" (
				call :error_restore
			)
